...
class Attention(nn.Module):
    def __init__(self,
        ...
        if mixer == 'Local' and HW is not None:
            hk = local_k[0]
            wk = local_k[1]
            mask = torch.ones([H * W, H + hk - 1, W + wk - 1]).float().to(device)
            for h in range(0, H):
                for w in range(0, W):
                    mask[h * W + w, h:h + hk, w:w + wk] = 0.
            mask_paddle = mask[:, hk // 2:H + hk // 2, wk // 2:W + wk //2].flatten(1)
            mask_inf = torch.full([H * W, H * W], -np.inf).float().to(device) ### 造成nan
    ...
    def forward(self, x):
        ...
        x = (attn.matmul(v)).permute((0, 2, 1, 3)).reshape((-1, N, C)).contiguous()
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class SVTRNet(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(torch.zeros([1, num_patches, embed_dim[0]], dtype=torch.float32), requires_grad=True).to(device)

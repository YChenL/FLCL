...
class LocalBlock(nn.Module):
    def __init__(self, dim, kernel_size=3, dilation=1, scale=4, drop_path=0.):
        super().__init__()
        self.res2conv = res2conv(dim, kernel_size, dilation, scale)     
...
class GlobalBlock(nn.Module):
    ...
    def __init__(self, dim, h=16, w=800):
        super().__init__()
        self.gf = GlobalFilter(dim, h=h, w=w) # Global-aware filters
...
class dscnnorg(nn.Module):
    def __init__(self, nIn, nOut, imgH, imgW, channels=(96, 192, 384), blocks=(1, 1, 2), heads=(1, 4, 8), r=[4, (2,1), 1], num_classes=1000, uniform_init=True):
        super().__init__()
        ...
        self.downsample3 = ConvDownsampling(inp=channels[2]//2, oup=channels[2]//2, r=(2,1))
        ...
        self.llayer4 = LocalBlock(channels[2]//2, kernel_size=3, scale=4, dilation=1)
        ...
        self.glayer4 = GlobalBlock(channels[2]//2, h=imgH//32,  w=imgW//4)

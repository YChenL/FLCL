...
class dscnn(nn.Module):
    def __init__(self, nIn, nOut, imgH, imgW, channels=(64, 128, 256, 512), n_blocks=[2,2,6,2], uniform_init=True):
        super().__init__()
        
        self.stem = FirstConvDownsampling(inp=nIn, oup=channels[0], r=(2,1))
        self.downsample1 = ConvDownsampling(inp=channels[0], oup=channels[1], r=(2,1))
        self.downsample2 = ConvDownsampling(inp=channels[1], oup=channels[2], r=(2,1))
        self.downsample3 = ConvDownsampling(inp=channels[2], oup=channels[3], r=(2,1))

        # local branch
        # self.llayer1 = LocalBlocks(n_blocks[0], channels[0]//2, kernel_size=3, scale=4, dilation=1) #默认8 8 8 C=1024; 尝试4 6 8 C=960
        self.llayer1 = LocalBlock(channels[0], kernel_size=3, scale=4, dilation=1) #默认8 8 8 C=1024; 尝试4 6 8 C=960
        self.llayer2 = LocalBlock(channels[1], kernel_size=3, scale=4, dilation=1)
        self.llayer3 = LocalBlock(channels[2], kernel_size=3, scale=4, dilation=1)
        self.llayer4 = LocalBlock(channels[3], kernel_size=3, scale=4, dilation=1)
        
        #global branch
        # self.glayer1 = GlobalBlocks(n_blocks[0], channels[0]//2, h=imgH//4,  w=imgW//4)
        self.glayer1 = GlobalBlock(channels[0], h=imgH//4,  w=imgW//4)
        self.glayer2 = GlobalBlock(channels[1], h=imgH//8,  w=imgW//4)
        self.glayer3 = GlobalBlock(channels[2], h=imgH//16, w=imgW//4)
        self.glayer4 = GlobalBlock(channels[3], h=imgH//32, w=imgW//4)
        
        # feature fusion
        dwconv_list, fc_list = [], []
        for i in range(len(channels)):
            dwconv_list.append(nn.Conv2d(2*channels[i], 2*channels[i], kernel_size=3, padding=1, groups=channels[i], bias=False))
            fc_list.append(nn.Conv2d(2*channels[i], channels[i], kernel_size=1, bias=False))   
             
        self.dwconv_list = nn.ModuleList(dwconv_list)
    ...
    def merge(self, l, g, idx):
        inputs = torch.cat((l, g), dim=1)
        f = self.dwconv_list[idx](inputs)
        f = f + inputs
        f = self.fc_list[idx](f)
        return f

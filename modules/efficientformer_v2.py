...
class Attention4D(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        _, _, res_h, res_w = v_local.shape # calculate resolution
        ...
        out = x.transpose(2, 3).reshape(B, self.dh, res_h, res_w) + v_local
...
def stem(in_chs, out_chs, act_layer=nn.ReLU):
    return nn.Sequential(
        nn.Conv2d(in_chs, out_chs // 2, kernel_size=3, stride=2, padding=(1,0)),
        nn.BatchNorm2d(out_chs // 2),
        act_layer(),
        nn.Conv2d(out_chs // 2, out_chs, kernel_size=3, stride=2, padding=(1,0)),
...
class LGQuery(torch.nn.Module):
    def __init__(self, in_dim, out_dim, resolution1, resolution2):
        ...
        self.pool = nn.AvgPool2d(1, (1,2), 0)
        self.local = nn.Sequential(nn.Conv2d(in_dim, in_dim, kernel_size=3, stride=(1,2), padding=1, groups=in_dim),
                                   )
...
class Attention4DDownsample(torch.nn.Module):
    def __init__(self, dim=384, key_dim=16, num_heads=8,
        ...
        self.N = self.resolution ** 2
        self.N2 = 2*(self.resolution2 ** 2)

        self.k = nn.Sequential(nn.Conv2d(dim, self.num_heads * self.key_dim, 1), #, stride=(2,1)
                               nn.BatchNorm2d(self.num_heads * self.key_dim), )
        self.v = nn.Sequential(nn.Conv2d(dim, self.num_heads * self.d, 1),       #, stride=(2,1)
                               nn.BatchNorm2d(self.num_heads * self.d),
                               )
        self.v_local = nn.Sequential(nn.Conv2d(self.num_heads * self.d, self.num_heads * self.d,
                                               kernel_size=3, stride=(2,1), padding=1, groups=self.num_heads * self.d),
                                     nn.BatchNorm2d(self.num_heads * self.d))
        ...
        points_ = list(itertools.product(
            range(self.resolution2), range(2*self.resolution2)))
        N = len(points)
        N_ = len(points_)
        attention_offsets = {}
        idxs = []
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        out = x.reshape(B, self.dh, res_h, res_w) + v_local 
        out = self.proj(out)
...
class EfficientFormerV2(nn.Module):
    def __init__(self, nIn, nOut, layers, embed_dims=None,
                 mlp_ratios=4, downsamples=None,
                 pool_size=3,
                 norm_layer=nn.BatchNorm2d, act_layer=nn.GELU,
                 num_classes=1000,
                 down_patch_size=3, down_stride=(2,1), down_pad=1,
                 drop_rate=0., drop_path_rate=0.,
                 use_layer_scale=True, layer_scale_init_value=1e-5,
                 fork_feat=False,
                 init_cfg=None,
        ...
        self.patch_embed = stem(nIn, embed_dims[0], act_layer=act_layer)
...
# @register_model
def efficientformerv2_s0(nIn, nOut, pretrained=False, **kwargs):
...
# @register_model
def efficientformerv2_s1(nIn, nOut, pretrained=False, **kwargs):
    model = EfficientFormerV2(
        nIn,
        nOut,
...
# @register_model
def efficientformerv2_s2(nIn, nOut, pretrained=False, **kwargs):
    model = EfficientFormerV2(
        nIn,
        nOut,
...
# @register_model
def efficientformerv2_l(nIn, nOut, pretrained=False, **kwargs):
    model = EfficientFormerV2(
        nIn,
        nOut,

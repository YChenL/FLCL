...
from utils import CTCLabelConverter, CTCLabelConverterForBaiduWarpctc, AttnLabelConverter, Averager, FL_InfoNCELoss, InfoNCELoss
...
def train(opt):
    ...
    if opt.CL: 
        from dataset_cl import Batch_Balanced_Dataset
    ...
    lamb     = 1 # set scale factor for balance between the STR object and CL object     
    ...
    for epoch in range(opt.max_epoch):
        ...
        for iter in range(train_dataset.total_iters()):
            ...
            # train part    
            if opt.CL:
                image_tensors, blur_image_tensors, labels = train_dataset.get_batch()
                view1 = image_tensors.to(device)  
                view2 = blur_image_tensors.to(device)  
            ...
            if 'CTC' in opt.Prediction:    
                ...
                if opt.CL: 
                    ...
                    logits, labels = FL_InfoNCELoss(feature, batch_size)
                ...
                # forward
                preds1, z1 = model(view1, text[:, :-1])  # align with Attention.forward 
                # compute OCR loss
                target = text[:, 1:]  # without [GO] Symbol
                cost   = criterion(preds1.view(-1, preds1.shape[-1]), target.contiguous().view(-1)) 
            
                if opt.CL:
                    preds2, z2 = model(view2, text[:, :-1])
                    # compute OCR loss
